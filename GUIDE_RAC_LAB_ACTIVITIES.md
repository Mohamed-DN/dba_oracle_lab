...
> - GoldenGate configured and working
> - RMAN backup configured
>
...
### 1.2 Database Checks (from oracle, sqlplus)
```sql
...
--Check recent alert log (latest errors)
...
--Use our script or direct query
...
## 🎯 Exercise 2: Real Time Performance Analysis (45 min)
...
### 2.2 Top SQL for resource consumption
```sql
...
### 2.4 Locks and blocking sessions
```sql
...
## 🎯 Exercise 3: Generate and Analyze an AWR Report (30 min)

> **Scenario**: You want to compare today's performance with yesterday's.
...
```sql
--Interactive (asks you begin/end snap_id):
@$ORACLE_HOME/rdbms/admin/awrrpt.sql

--Non-interactive (use community script):
...
### 3.3 Analyze the report
Look for these key sections in the report:
1. **Top 10 Foreground Events** — major bottlenecks
...
> 📋 **Go further**: Generate an ASH report for a specific interval:`@$ORACLE_HOME/rdbms/admin/ashrpt.sql`
...
--Logfile sync diagnosis (essential!)
...
--Regenerate statistics on a schema
...
> **Scenario**: Planned maintenance simulation — switch to DR site.
...
--4. Check GoldenGate (must continue to work!)
--from shell: ggsci > info all

--5. Switchback (return to the original situation)
...
## 🎯 Exercise 8: Problem Simulation and Kill Session (15 min)
...
--4. If it doesn't die, kill at OS level
...
## 🎯 Exercise 9: RAC Monitoring — Cache Fusion and Interconnect (20 min)

> **Scenario**: Verify that the communication between the 2 RAC nodes is efficient.
...
> **Scenario**: Verify that GoldenGate replication is working properly.
...
--4. Check on the target (after a few seconds)
...
|Disk space/tablespace| `tablespace.sql`, `showtbs.sql`, `showdf.sql`, `undo_space.sql` |

...
## 1. 🔍 Health Check Rapido — "Come sta il mio DB?"

Esegui queste query dopo ogni fase del lab per verificare lo stato del database.
...
> **Perché?** Mostra tutto in un colpo: il DB è aperto? È in archivelog? È primario o standby? Il force logging è attivo? Questa query da sola ti dice il 50% dello stato del DB.
...
> **Perché?** Se qualcuno si lamenta che il DB è lento, questa query mostra CHI sta facendo COSA e da QUANTO TEMPO.

### 2.2 Query più pesanti (Top SQL)
...
> **Perché?** I wait events sono il "linguaggio" di Oracle per dirti cosa lo rallenta. Se vedi `db file sequential read` = troppe letture da disco. Se vedi `log file sync` = il disco dei redo è lento. Se vedi `gc buffer busy` = contesa tra nodi RAC.
...
> **Quando preoccuparsi?** Se `pct_used` > 85%, devi aggiungere spazio. Se > 95%, è urgente!
...
> **Status**: `CURRENT` = in uso ora, `ACTIVE` = necessario per recovery, `INACTIVE` = può essere sovrascritto.

### 5.2 Generazione Redo (per capire il carico)
...
## 6. 👥 Utenti e Sicurezza

### 6.1 Utenti del Database
...
## 8. 🔧 Comandi DBA di Tutti i Giorni
...
> **Regola**: Usa `SHUTDOWN IMMEDIATE` nel 99% dei casi. `ABORT` solo se IMMEDIATE è bloccato.
...
> **Leggi l'alert log OGNI GIORNO.** È il primo posto dove cercare problemi.
...
| `sessions.sql` | Monitoring | ✅ **Essenziale** | Tutte le sessioni |
| `top_sql.sql` | Monitoring | ✅ **Essenziale** | Query più costose |
| `locked_objects.sql` | Monitoring | ✅ **Essenziale** | Oggetti con lock |
| `ts_full.sql` | Monitoring | ✅ **Essenziale** | Spazio tablespace |
| `datafiles.sql` | Monitoring | ✅ **Utile** | Info datafile |
| `logfiles.sql` | Monitoring | ✅ **Utile** | Stato redo log |
| `redo_by_day.sql` | Monitoring | ✅ **Utile** | Volume redo per giorno |
| `redo_by_hour.sql` | Monitoring | ✅ **Utile** | Volume redo per ora |
...
| `system_events.sql` | Monitoring | ✅ **Utile** | Wait events di sistema |
| `parameters_non_default.sql` | Monitoring | ✅ **Utile** | Parametri modificati |
| `patch_registry.sql` | Monitoring | ✅ **Utile** | Patch installate |
| `cache_hit_ratio.sql` | Monitoring | ✅ **Utile** | Efficienza buffer cache |
| `free_space.sql` | Monitoring | ✅ **Utile** | Spazio libero |
| `longops.sql` | Monitoring | ✅ **Utile** | Operazioni lunghe |
| `recovery_status.sql` | Monitoring | ✅ **Utile** | Stato recovery |
| `db_info.sql` | Monitoring | ✅ **Essenziale** | Info database |
| `sessions_rac.sql` | RAC | ✅ **Essenziale RAC** | Sessioni per tutte le istanze |
| `locked_objects_rac.sql` | RAC | ✅ **Essenziale RAC** | Lock su tutte le istanze |
| `session_waits_rac.sql` | RAC | ✅ **Essenziale RAC** | Wait su tutte le istanze |
| `monitor_memory_rac.sql` | RAC | ✅ **Utile RAC** | Uso memoria per istanza |
| `compile_all.sql` | Misc | 🔶 **Situazionale** | Ricompila tutto (dopo patch) |
| `login.sql` | Misc | ✅ **Consigliato** | Personalizza SQL*Plus prompt |
...
| Script | Motivo |
|---|---|
| `drop_all.sql` | 🚨 Pericoloso! Cancella TUTTI gli oggetti dello schema |
...
Esegui tutte queste verifiche come check finale del tuo ambiente:

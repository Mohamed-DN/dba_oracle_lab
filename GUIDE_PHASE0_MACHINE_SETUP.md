...
| Macchina | Tipo | RAM | CPU | Disco OS | Disco /u01 | ASM discs |
|---|---|---|---|---|---|---|
...
| `racstby2-vip` | VIP Standby No.2 | 192.168.56.114 | — | Gestito dal CRS |
...
|---|---|---|---|
| **OPatch** (utility) | **6880880** | [Scarica qui](https://updates.oracle.com/Orion/PatchDetails/process_form?patch_num=6880880) | ALWAYS update before each RU. **Note**: For January 2026 patch you need v12.2.0.1.48+ |
...
fdisk /dev/sdb
```
*(Premi la sequenza di tasti: `n` [New], `p` [Primaria], `1` [Numero 1], `Invio` [Primo settore default], `Invio` [Ultimo settore default], `w` [Write and save])*
...
> - **Campo 6 (Pass)**: Indica l'ordine in cui il tool `fsck` will scan disks on startup. It was used with old ext3/ext4 filesystems `1` per il root e `2` for the other discs. **But XFS doesn't use fsck at boot!** XFS handles consistency (journaling) internally at mount time.
> 
...
> 
> Therefore, our educational choice to use ASMLib (`oracleasm`) nel laboratorio non solo facilita enormemente l'insegnamento rispetto a UDEV, ma si allinea perfettamente all'attuale "ritorno di fiamma" di Oracle verso questo componente!
...
#### Step 1: Esegui `fdisk` in Manual mode (Educational)
Dal terminale MobaXterm su `rac1` as a user `root`, lancia `fdisk` per il primo disco:
...
At the end of Phase 1, from your `rac1` spento, eseguirai queste clonazioni in cascata, generando sempre **nuovi indirizzi MAC**:

# 08 - TDE & Security

Script e note operative per audit e sicurezza Oracle nel toolkit.

## Contenuto disponibile

Questa area contiene script SQL orientati a audit trail e controlli sessione, ad esempio:

- `dba_audit_trail.sql`
- `dba_audit_session.sql`
- `dba_audit_session_recent.sql`
- `dba_table_audit_flags.sql`
- `show_session_audit.sql`
- `ua-audit-log-cleanup-job.sql`

## Obiettivi pratici

- verificare eventi audit e accessi privilegiati
- controllare volumi log audit
- impostare routine di cleanup

## Riferimenti

- [GUIDA_ATTIVITA_DBA.md](../../../04_governance_learning/03_esami_e_carriera/GUIDA_ATTIVITA_DBA.md)
- [GUIDA_MAA_BEST_PRACTICES.md](../../../02_core_dba/04_high_availability_and_rac/GUIDA_MAA_BEST_PRACTICES.md)

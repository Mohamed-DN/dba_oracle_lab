...
ADMINISTER KEY MANAGEMENT CREATE KEYSTORE IDENTIFIED BY "Wallet#Pass123";

-- Apri keystore
ADMINISTER KEY MANAGEMENT SET KEYSTORE OPEN IDENTIFIED BY "Wallet#Pass123";
```

### 3) Creazione master key TDE (con backup)

```sql
ADMINISTER KEY MANAGEMENT SET KEY IDENTIFIED BY "Wallet#Pass123" WITH BACKUP;
```

In CDB/PDB usa `CONTAINER=ALL` quando richiesto dalla policy operativa:

```sql
ADMINISTER KEY MANAGEMENT SET KEY IDENTIFIED BY "Wallet#Pass123" WITH BACKUP CONTAINER=ALL;
```
